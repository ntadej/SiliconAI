...
class TransformerBase(nn.Module):
    ...
    def __init__(self, config: Configuration) -> None:
        ...
        self.embedding_dim = config.model.model_dim
        self.model_dim = (
            config.model.model_dim * len(config.data.input_dim)
            if self.cat
            and isinstance(
                config.data.input_dim,
                list,
            )
            else config.model.model_dim
        )
        self.output_dim = (
...
class ChainTransformer(TransformerBase):
    ...
    def predict(
        ...
        **kwargs: Unpack[TransformerPredictParams],
        ...
        for i in range(self.config.model.sequence_length - input_tensor.size(1)):
            column = i % ncolumns
