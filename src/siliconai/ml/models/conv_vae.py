"""Convolutional variational autoencoder models."""
...
from siliconai.ml.models.common import SequentialConv2d

if TYPE_CHECKING:
    from siliconai.cli.config import Configuration


def calculator(i: int) -> int:
    """Calculate size."""
    k = 3
    p = 1
    s = 2
...
class Encoder(nn.Module):
    ...
    def __init__(self, config: Configuration) -> None:
        ...
        input_dim = config.data.input_dim[0]
        size = config.data.input_dim[1]
        for _ in config.model.encoder_layers:
        ...
        self.model_mu = nn.Linear(
            config.model.encoder_layers[-1] * size * size,
...
class Decoder(nn.Module):
    ...
    def __init__(self, config: Configuration) -> None:
        ...
        self.model_input = nn.Linear(
            latent_dim,
            config.model.decoder_layers[0] * size * size,
        )
        ...
        self.model_main = SequentialConv2d(
            [*config.model.decoder_layers, config.model.decoder_layers[-1]],
            config.model.activation,
            config.model.activation_parameters,
            config.model.batch_norm,
            config.model.dropout,
